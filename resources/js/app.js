...
document.querySelectorAll('[data-repeat-editor]').forEach(editor=>{let sequence=0;editor.querySelector('[data-repeat-add]').addEventListener('click',()=>{const holder=document.createElement('div');holder.innerHTML=editor.querySelector('template').innerHTML.replaceAll('__KEY__','new_'+Date.now()+'_'+sequence++);editor.querySelector('[data-repeat-list]').append(holder.firstElementChild);});editor.addEventListener('click',event=>event.target.closest('[data-repeat-remove]')?.closest('[data-repeat-row]')?.remove());});
