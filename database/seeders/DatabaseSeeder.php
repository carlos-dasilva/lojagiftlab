...
class DatabaseSeeder extends Seeder
{
    ...
    {
        $categories = collect(['Impressão 3D', 'Action Figures', 'Geek', 'Games Retrô', 'Presentes', 'Casa', 'Cozinha', 'Plantas', 'Decoração', 'Utilidades', 'Moda íntima', 'Outros'])->mapWithKeys(fn ($name, $i) => [$name => Category::firstOrCreate(['slug' => str($name)->slug()], ['name' => $name, 'order' => $i, 'active' => true])]);
        $samples = [
        ...
        $demoChannel = SalesChannel::firstOrCreate(['slug' => 'demonstracao'], ['name' => 'Demonstração (sem venda)', 'color' => '#7139C6', 'active' => true]);
        foreach ($samples as $i => [$name,$category,$price,$discount,$description]) {
            $product = Product::firstOrCreate(['slug' => str($name)->slug()], ['name' => $name, 'short_description' => $description, 'description' => $description."\n\nProduto fictício criado para demonstração do catálogo.", 'cost_price' => round($price * .45, 2), 'sale_price' => null, 'discount_percentage' => 0, 'stock' => 10, 'condition' => 'new', 'featured' => $i < 4, 'is_new' => $i > 3, 'customizable' => $name === 'Presente Personalizado', 'made_to_order' => $name === 'Presente Personalizado', 'status' => 'published', 'order' => $i]);
            if ($product->wasRecentlyCreated) {
                $product->allSalesLinks()->create(['sales_channel_id' => $demoChannel->id, 'url' => 'https://example.com/gift-lab-demo/'.$product->slug, 'price' => round($price * (1 - $discount / 100), 2), 'original_price' => $discount > 0 ? $price : null, 'active' => true]);
            }
        ...
        foreach ([['Instagram', '#FF3D5F'], ['WhatsApp', '#19a974'], ['Mercado Livre', '#3483FA'], ['Shopee', '#EE4D2D']] as $i => [$name,$color]) {
            SalesChannel::firstOrCreate(['slug' => str($name)->slug()], ['name' => $name, 'color' => $color, 'active' => true, 'order' => $i]);
        }
        ...
        foreach ([['Como faço para comprar?', 'Escolha um canal disponível na página do produto. A compra acontece no ambiente externo selecionado.'], ['Vocês fazem produtos personalizados?', 'Quando um produto aceitar personalização, essa informação aparecerá em destaque. Entre em contato para conversar sobre sua ideia.'], ['Os produtos do seed são reais?', 'Não. São exemplos fictícios para demonstração; confirme os produtos reais com a loja.']] as $i => [$q,$a]) {
            FaqItem::firstOrCreate(['question' => $q], ['answer' => $a, 'order' => $i, 'active' => true]);
        }
    }
}
